        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('penjualan.update', $penjualan->id) }}" id="formPenjualan">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" id="nomor_nota" value="{{ $penjualan->nomor_nota }}" readonly>
                                </div>

                                <div class="form-group mb-3">
                                    <label for="tanggal">Tanggal:</label>
                                    <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal" value="{{ old('tanggal', $penjualan->tanggal) }}" required>
                                    @error('tanggal')
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control @error('jenis_penjualan') is-invalid @enderror" id="jenis_penjualan" name="jenis_penjualan" required>
                                        <option value="tanpa_resep" {{ $penjualan->jenis_penjualan == 'tanpa_resep' ? 'selected' : '' }}>Tanpa Resep</option>
                                        <option value="dengan_resep" {{ $penjualan->jenis_penjualan == 'dengan_resep' ? 'selected' : '' }}>Dengan Resep</option>
                                    </select>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="tabelDetailObat">
                                ...
                                <tbody>
                                    ...
                                    <tr id="row-{{ $index }}">
                                        <td>
                                            <select class="form-control obat-select" name="obat_id[]" data-row="{{ $index }}" required>
                                                <option value="">Pilih Obat</option>
                                                @foreach ($obats as $obat)
                                                    <option value="{{ $obat->id }}" 
                                                            data-harga="{{ $obat->harga_jual }}" 
                                                            data-stok="{{ $obat->stok + ($obat->id == $detail->obat_id ? $detail->jumlah : 0) }}"
                                        ...
                                        <td>
                                            <span class="harga-display" id="harga-display-{{ $index }}">{{ number_format($detail->harga_satuan, 0, ',', '.') }}</span>
                                            <input type="hidden" class="harga-obat" id="harga-{{ $index }}" value="{{ $detail->harga_satuan }}">
                                        </td>
                                        <td>
                                            <span class="stok-display" id="stok-display-{{ $index }}">{{ $detail->obat->stok + $detail->jumlah }}</span>
                                        </td>
                                        <td>
                                            <input type="number" class="form-control jumlah-obat" name="jumlah[]" id="jumlah-{{ $index }}" min="1" value="{{ $detail->jumlah }}" data-row="{{ $index }}" required>
                                        </td>
                                        <td>
                                            <span class="subtotal-display" id="subtotal-display-{{ $index }}">{{ number_format($detail->subtotal, 0, ',', '.') }}</span>
                                            <input type="hidden" class="subtotal-obat" id="subtotal-{{ $index }}" value="{{ $detail->subtotal }}">
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-danger btn-sm hapus-baris" data-row="{{ $index }}" {{ $index == 0 && count($penjualan->details) == 1 ? 'disabled' : '' }}>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td>
                                            <span id="total-penjualan">{{ number_format($penjualan->total_harga, 0, ',', '.') }}</span>
                                            <input type="hidden" name="total_harga" id="total-harga" value="{{ $penjualan->total_harga }}">
                                        </td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                        ...
                        <div class="row">
                            <div class="col-md-12 d-flex justify-content-between">
                                <a href="{{ route('penjualan.show', $penjualan->id) }}" class="btn btn-secondary">Kembali</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('tambahBaris').addEventListener('click', function() {
            ...
            const template = document.querySelector('#tabelDetailObat tbody tr:first-child').cloneNode(true);
            template.id = 'row-' + rowCount;
            
            // Update attributes with new row index
            template.querySelectorAll('[data-row]').forEach(function(el) {
        ...
        document.getElementById('formPenjualan').addEventListener('submit', function(e) {
            ...
            document.querySelectorAll('.jumlah-obat').forEach(function(input) {
                ...
                if (jumlah > stok) {
                    valid = false;
                    const obatSelect = document.querySelector('.obat-select[data-row="' + rowId + '"]');
                    const obatName = obatSelect.options[obatSelect.selectedIndex].text;
